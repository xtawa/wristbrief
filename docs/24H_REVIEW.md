...
- Wear OS Tile-host activation coverage uses the platform debug surface on both CI Wear profiles to add, index, show and remove both WristBrief Tile providers (`Latest briefs` and `Continue listening`), requiring successful host responses at each step; CI also taps the host-rendered center card for each Tile and requires `MainActivity` to become the resumed activity, proving the Tile launch action through the real emulator host rather than only inspecting the ProtoLayout action;
...
- Wear OS small-round emulator execution at system font scale `1.30`, including navigation/state-restoration, injected rotary scrolling, MediaSession controller reconnect/state-continuity, service-recreation persisted-resume coverage, installed Tile/Complication provider contract/preview checks, platform Tile-host add/show/remove activation for both WristBrief Tile providers, and host-rendered Tile center-card tap launch into `MainActivity`;
- Wear OS large-round emulator execution at the default font scale, including the same instrumentation suite, rotary-scroll assertion, Tile-host activation checks and host-rendered Tile tap-launch assertion;
- Mobile JVM tests;
...
CI #344 verified commit `99c01faa1a375bcd30927393341979a86a412323`: release guard, Gateway typecheck/Vitest, Wear and Mobile JVM/debug builds, unsigned release APK/AAB builds, Mobile managed-device instrumentation, and both Wear small-round (`1.30` font scale) and large-round instrumentation completed successfully. The Wear jobs additionally proved that the system Tile host accepts, indexes, displays and removes both registered WristBrief Tile providers and that tapping each host-rendered Tile center card launches WristBrief into a resumed `MainActivity`.

These smoke suites prove launch/recreation/navigation, configured large-font layout interaction, deterministic Compose rotary-scroll handling, MediaSession-service connection/lifecycle behavior, persisted resume after service reconstruction, prepared and actively-playing local-media controller reconnect continuity, installed Wear surface provider registration/preview contracts, system Tile-host provider activation and Tile tap-launch on both emulator profiles, and the JVM-level Data Layer validation/state semantics now checked into the repository. They do **not** substitute for physical-crown hardware tuning, Bluetooth/noisy-route/audio-focus behavior, a real paired-device disconnect/reconnect transport exercise, pixel/readability assertions for the host-rendered Tile/Complication surfaces, complication-host placement/tap behavior, meaningful-change Tile/Complication update-propagation checks, Play validation, or the broader physical-device matrix below.
...
- expand Android/Wear instrumentation beyond the current launch/recreation/navigation/rotary/media/provider-contract/Tile-host/tap checks into additional deterministic interaction flows where practical;
...
- verify actual Tile and complication host rendering/readability and update propagation on Wear OS. CI now proves that the Wear OS Tile host can add/show/remove both Tile providers and that tapping each host-rendered Tile center card launches WristBrief on both emulator profiles; provider registration/permissions/metadata and complication preview construction are also covered. Pixel/readability, meaningful-change update propagation, and complication-host placement/tap behavior remain unclaimed.
...
1. Continue deterministic Wear surface coverage where the emulator host exposes stable automation hooks, focusing next on meaningful-change Tile/Complication update propagation and complication-host placement/tap behavior without replacing real-device visual/readability checks with brittle pixel assumptions.
