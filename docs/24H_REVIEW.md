...
- Wear OS Tile-host activation coverage now uses the platform debug surface on both CI Wear profiles to add, index, show and remove both WristBrief Tile providers (`Latest briefs` and `Continue listening`), requiring successful host responses at each step;
...
- Wear OS small-round emulator execution at system font scale `1.30`, including navigation/state-restoration, injected rotary scrolling, MediaSession controller reconnect/state-continuity, service-recreation persisted-resume coverage, installed Tile/Complication provider contract/preview checks, and platform Tile-host add/show/remove activation for both WristBrief Tile providers;
- Wear OS large-round emulator execution at the default font scale, including the same instrumentation suite, rotary-scroll assertion and Tile-host activation checks;
- Mobile JVM tests;
...
- expand Android/Wear instrumentation beyond the current launch/recreation/navigation/rotary/media/provider-contract/Tile-host-activation checks into additional deterministic interaction flows where practical;
...
- verify actual Tile and complication host rendering/readability, tap launch behavior, and update propagation on Wear OS. CI now proves that the Wear OS Tile host can add/show/remove both Tile providers on both emulator profiles, while provider registration/permissions/metadata and complication preview construction are also covered; pixel/readability, tap-launch, meaningful-change update propagation and complication-host placement remain unclaimed.
...
1. Continue deterministic Wear surface coverage where the emulator host exposes stable automation hooks, focusing on Tile/Complication tap/update behavior without replacing real-device visual/readability checks with brittle pixel assumptions.
