...
export async function restorePlayPurchase(
  ...
  if (parsed.packageName !== config.packageName || !config.productIds.has(parsed.productId)) {
    return result(400, "purchase_not_allowed");
  }
  ...
  const verified = await config.verifier.verifySubscription({
    packageName: config.packageName,
    purchaseToken: parsed.purchaseToken
  });
  if (verified.packageName !== config.packageName || verified.productId !== parsed.productId || !config.productIds.has(verified.productId)) {
    return result(400, "purchase_mismatch");
  }
...
export async function processPlayRtdn(request: Request, env: BillingServerEnv): Promise<BillingResult> {
  ...
  // RTDN is only a change signal. Entitlement is derived from a fresh Play API verification.
  const verified = await config.verifier.verifySubscription({
    packageName: config.packageName,
    purchaseToken: notification.purchaseToken
  });
  if (verified.packageName !== config.packageName || !config.productIds.has(verified.productId)) {
    return result(400, "purchase_mismatch");
  }
...
export class FakeGooglePlayPurchaseVerifier implements GooglePlayPurchaseVerifier {
  ...
  async verifySubscription(): Promise<VerifiedPlaySubscription> {
    this.calls += 1;
...
export class FakePubSubPushAuthenticator implements PubSubPushAuthenticator {
  ...
  async verify(): Promise<boolean> { return this.allowed; }
}
...
function configuredBilling(env: BillingServerEnv) {
  ...
  const verifier = env.PLAY_PURCHASE_VERIFIER;
...
function publicSubscription(value: VerifiedPlaySubscription) {
  return { productId: value.productId, status: value.status, expiresAt: value.expiresAt };
}

function result(status: number, error: string): BillingResult {
