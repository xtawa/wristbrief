...
describe("WristBrief gateway", () => {
  ...
  it("rejects non-HTTPS provider configuration", async () => {
    const response = await worker.fetch(summaryRequest("hello"), { ...env, AI_BASE_URL: "http://provider.example/v1" });
  ...
  it("returns compatibility summary plus validated structured brief", async () => {
    const upstreamFetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      expect(init?.headers).toMatchObject({ "Content-Type": "application/json", "Authorization": "Bearer provider-secret" });
      return new Response(JSON.stringify({ choices: [{ message: { content: JSON.stringify(structured) } }] }), {
        status: 200,
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ summary: structured.brief, model: "test-model", structured });
  ...
  it("rejects malformed model output after one repair attempt", async () => {
    const upstreamFetch = vi.fn(async () => new Response(JSON.stringify({ choices: [{ message: { content: "bad" } }] }), { status: 200 }));
    vi.stubGlobal("fetch", upstreamFetch);
    const response = await worker.fetch(summaryRequest("hello"), env);
    expect(response.status).toBe(502);
