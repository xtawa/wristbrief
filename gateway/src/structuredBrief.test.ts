...
describe("structured brief schema", () => {
  it("parses and normalizes valid JSON", () => {
    expect(parseStructuredBrief(JSON.stringify({ ...validBrief, tiny: "  Tiny summary.  " }))).toEqual(validBrief);
  });
  ...
  it("bounds arrays and text so unchecked model output cannot reach the client", () => {
