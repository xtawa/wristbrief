...
export function buildStructuredBriefPrompt(title: string | undefined, content: string): PromptMessages {
  return {
    system: [
      "You summarize RSS articles and podcast transcripts for a Wear OS reader.",
      ...
      "Return JSON only, with exactly these fields: tiny, brief, bullets, topics, sourceLanguage, outputLanguage, schemaVersion, promptVersion.",
      "tiny: one glanceable sentence. brief: concise factual summary. bullets: 1-6 key facts. topics: 0-8 short topic labels.",
      "sourceLanguage and outputLanguage must be short language tags/names. Preserve important names, numbers, and dates.",
...
export function parseStructuredBrief(raw: string): StructuredBrief | null {
  ...
  if (!tiny || !brief || !bullets || !topics || !sourceLanguage || !outputLanguage) return null;
