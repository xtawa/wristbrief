        ...
        try {
            client.newCall(requestBuilder.build()).execute().use { response ->
                if (!response.isSuccessful) {
                    throw PhoneSummaryRequestException(
                        mapPhoneSummaryStatus(
                            status = response.code,
                            isByokRequest = headers.containsKey(BYOK_API_KEY_HEADER),
                        ),
                    )
                }
...
internal fun mapPhoneSummaryStatus(status: Int, isByokRequest: Boolean): PhoneSummaryFailure = when {
    status == 401 || status == 403 -> PhoneSummaryFailure.Unauthorized
    status == 422 && isByokRequest -> PhoneSummaryFailure.ByokProviderUnauthorized
    status == 429 -> PhoneSummaryFailure.Quota
    status == 502 || status == 503 || status == 504 -> PhoneSummaryFailure.ProviderUnavailable
